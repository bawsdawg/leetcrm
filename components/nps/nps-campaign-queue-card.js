...
import { CLIENTS, NPS_TEMPLATES, NPS_UPCOMING_SENDS } from "@/lib/crm/static-data";
...
export function NpsCampaignQueueCard() {
  return (
    <section className="rounded-2xl border border-border bg-surface-card p-4 shadow-inset-card md:p-5">
      <div className="flex flex-col gap-1 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-xl font-sans text-[11px] leading-snug text-fg-muted">
            Synk til marketing automation og account owners — erstattes af `NpsCampaign` + webhook status.
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 overflow-x-auto">
        <table className="w-full min-w-[560px] border-collapse text-left text-[12px]">
          ...
          <tbody>
            {NPS_UPCOMING_SENDS.map((row) => {
              const client = CLIENTS.find((c) => c.id === row.clientId);
              const tmpl = NPS_TEMPLATES.find((t) => t.id === row.templateId);
              const cfg = STATUS_DA[row.status] ?? STATUS_DA.draft;
              ...
              return (
                <tr key={row.id} className="border-b border-border-soft last:border-0">
                  ...
                  <td className="py-2.5 pr-3 font-sans text-[12px] text-fg-muted">{tmpl?.name ?? row.templateId}</td>
                  <td className="py-2.5">
                    <span
                      className={cn(
